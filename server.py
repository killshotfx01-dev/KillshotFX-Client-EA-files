...
@app.route("/calc/oco-sizing", methods=["POST"])
def calc_oco_sizing():
    """
    Works out the lot size for a margin-based OCO order: margin_pct% of
    account balance, divided by the broker's margin requirement per lot
    (which the client fetches locally from MT5 and sends here, since it's
    broker/symbol-specific and MT5 access is only available client-side).
    """
    data = request.get_json(force=True, silent=True) or {}
    license_key = data.get("license_key", "")
    hardware_id = data.get("hardware_id", "")
    ...
    if margin_per_lot <= 0:
        return jsonify({"status": "error", "message": "margin_per_lot must be greater than 0."}), 400

    target_margin = balance * (margin_pct / 100.0)
    raw_lots = target_margin / margin_per_lot

    return jsonify({"status": "ok", "raw_lots": raw_lots})
...
def calc_be_plan():
    ...
    try:
        offset_pips = float(data["offset_pips"])
