...
Endpoints:
    ...
    POST /whitelist               - client-facing: returns the current approved XM account list
    ...
    POST /admin/whitelist/set    - replaces the ENTIRE whitelist (requires ADMIN_SECRET)
    POST /admin/whitelist/add    - adds one account to the whitelist (requires ADMIN_SECRET)
    POST /admin/whitelist/remove - removes one account from the whitelist (requires ADMIN_SECRET)
    GET  /                       - health check
"""
import hashlib
import hmac
...
def get_whitelist():
    """
    Client-facing: returns the current list of approved XM account numbers.
    Requires a valid, active license (same check as /activate), so the list
    isn't reachable by anyone who doesn't already hold a real license key -
    unlike a public/GitHub-hosted file, there's no static secret embedded in
    client code that could be extracted to bypass this.
    """
    data = request.get_json(force=True, silent=True) or {}
    license_key = data.get("license_key", "")
    hardware_id = data.get("hardware_id", "")
    ...
    conn = get_db()
    rows = conn.execute("SELECT account_number FROM whitelist_accounts").fetchall()
    conn.close()
    return jsonify({"status": "ok", "accounts": [r["account_number"] for r in rows]})
...
@app.route("/admin/whitelist/set", methods=["POST"])
def set_whitelist():
    """
    Replaces the ENTIRE whitelist with the given list of account numbers.
    Run this after every fresh export from your XM Partner Area - see the
    updated build_whitelist.py, which can push straight here instead of
    writing a local JSON file.
    """
    data = request.get_json(force=True, silent=True) or {}
    if data.get("admin_secret") != ADMIN_SECRET:
        return jsonify({"status": "error", "message": "Invalid admin secret."}), 403
    ...
    if not isinstance(accounts, list):
        return jsonify({"status": "error", "message": "accounts must be a list of account numbers."}), 400

    try:
        accounts = [int(a) for a in accounts]
    except (TypeError, ValueError):
        return jsonify({"status": "error", "message": "Every account number must be an integer."}), 400

    conn = get_db()
    conn.execute("DELETE FROM whitelist_accounts")
    conn.executemany(
        "INSERT INTO whitelist_accounts (account_number, added_at) VALUES (?, ?)",
        [(a, int(time.time())) for a in accounts],
    )
    conn.commit()
    conn.close()
...
def add_whitelist_account():
    """Adds a single account without touching the rest of the list - handy
    for approving one new client without waiting for the next full export."""
    data = request.get_json(force=True, silent=True) or {}
    if data.get("admin_secret") != ADMIN_SECRET:
        return jsonify({"status": "error", "message": "Invalid admin secret."}), 403
    ...
    try:
        ...
        return jsonify({"status": "error", "message": "account_number (integer) is required."}), 400

    conn = get_db()
    conn.execute(
        "INSERT OR IGNORE INTO whitelist_accounts (account_number, added_at) VALUES (?, ?)",
        (account_number, int(time.time())),
    )
    conn.commit()
    conn.close()
    return jsonify({"status": "ok", "account_number": account_number})


@app.route("/admin/whitelist/remove", methods=["POST"])
def remove_whitelist_account():
    data = request.get_json(force=True, silent=True) or {}
    if data.get("admin_secret") != ADMIN_SECRET:
        return jsonify({"status": "error", "message": "Invalid admin secret."}), 403
    ...
    conn = get_db()
    conn.execute("DELETE FROM whitelist_accounts WHERE account_number = ?", (account_number,))
    conn.commit()
    conn.close()
    return jsonify({"status": "ok", "account_number": account_number})
